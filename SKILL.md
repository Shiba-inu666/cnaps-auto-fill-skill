...
description: Codex 专用的中国银行开户行行号补全与核对技能。用于在 Excel、CSV 或 TSV 中依据“开户行”填写或核对行号；只读取两列，只改行号值，完整保留 Excel 原格式，并把异常和改动反馈在 Codex 对话中。
---
...
- 使用能原样保留工作簿格式的写入方式，不得重建或转换工作簿；无法保证时停止并说明。
...
8. 只设置行号单元格的值，不设置或复制样式。除非用户明确要求自动更正，否则已有值即使错误也不覆盖。默认另存新文件；完成后只复查开户行和行号两列，不渲染或检查整张表。
