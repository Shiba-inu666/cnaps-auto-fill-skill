...
4. 仅当结果为 `auto_fill_ready` 时处理：空白单元格写入 `autofill_code`；已有行号直接与 `autofill_code` 比较，一致则记为正确，不一致则记为错误但不自动覆盖。
...
7. 证据不足时标记为无法确认，不猜测对错；不对已有行号单独执行位数或格式检查。
...
回复保持简短：提供输出文件，以及补全、核对正确、不一致、无法确认和留空的数量。问题项只列工作表、行号、开户行、现有值、核验值及原因；除非用户要求，不新增完整审计表。
