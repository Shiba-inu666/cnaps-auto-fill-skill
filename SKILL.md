...
4. 仅当结果为 `auto_fill_ready` 时处理：空白单元格写入 `autofill_code`；已有值含非数字字符则记为格式错误，纯数字则直接与 `autofill_code` 比较，一致记为正确，不一致记为错误但不自动覆盖。
...
7. 证据不足时标记为无法确认，不猜测对错。格式检查只判断是否为纯数字，不检查位数。
8. 新行号按文本写入。除非用户明确要求自动更正，否则已有值即使错误也不覆盖。默认另存新文件；完成后只复查开户行和行号两列，不渲染或检查整张表。
...
回复保持简短：提供输出文件，以及补全、`正确`、`格式错误`、`不一致`、`无法确认`和留空的数量。问题项只列工作表、行号、开户行、现有值、核验值及原因；除非用户要求，不新增完整审计表。
