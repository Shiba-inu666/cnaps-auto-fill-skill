...
description: Codex 专用的中国银行开户行行号补全技能。用于在 Excel、CSV 或 TSV 中依据“开户行”填写或核验 12 位 CNAPS／人行行号；定位列后只读取开户行和行号两列，不访问其他业务数据。
---
...
2. 只查看行号为空的行，并读取同一行的开户行名称；已有行号不覆盖。
3. 对开户行名称去重，批量查询本地注册库：

   ```bash
   python3 scripts/cnaps_registry.py lookup --name '开户行一' --name '开户行二'
   ```

4. 仅当结果为 `auto_fill_ready` 且 `autofill_code` 是 12 位数字时直接填写。
...
7. 行号按文本写入。默认另存新文件；完成后只复查开户行和行号两列，不渲染或检查整张表。
...
回复保持简短：提供输出文件、填写数、留空数，以及兜底、旧称、冲突或未找到的行。问题项只列工作表、行号、开户行、写入行号或留空原因；除非用户要求，不新增完整审计表。
